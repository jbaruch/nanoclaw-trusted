...
The shared-state table stays — `#100` called it the authoritative part — but as an index of owning plugins rather than a second copy of their contracts. The column names the plugin, not the owner skill: `stateful-artifacts` puts the single-owner-skill declaration in that plugin's own `state-schema.md`, and naming it a second time from here is how this file went stale before. `session-state.json` is the exception, spelled out in full because its authority lives in this repo. `session-state.json`'s owner skill is this plugin's `tessl__trusted-memory` (contract in `skills/trusted-memory/state-schema.md`, implemented by `register-session.py`); `calendar-state.json`, `morning-brief-pending.json`, and `system-health-dismissed.json` are `nanoclaw-admin`'s; `cfp-state.json` is `nanoclaw-conferences`'. Added a note that cross-trust-tier skills belong under `/workspace/state/<skill-name>/` instead, since `/workspace/group/` is RO for untrusted.
...
Two review findings corrected the first draft. It credited `system-status` with `/workspace/group/system-health-dismissed.json` on the strength of a filename grep that had in fact matched that skill's own "What this skill is NOT" disclaimer — the file is `nanoclaw-admin` `heartbeat`'s, and `system-status` writes no file at all, emitting its report on stdout. The draft also labelled its table a reader/writer contract while giving only plugin names and per-repo reference counts, which do not establish ownership. Counts are gone: the table is now explicitly an index, pointing at the plugin whose owner skill holds each contract, because `coding-policy: stateful-artifacts` puts the schema next to the owner skill and a second copy of a four-repo contract maintained from inside one repo is the exact rot this rewrite is undoing. Non-owner writers are recorded as such rather than as co-owners: admin's `heartbeat` and `check-email` write individual fields on `session-state.json` under a mandatory `fcntl.LOCK_EX` on the `.lock` file, and writing a field is not schema ownership — the shape change stays the owner skill's, and readers meeting an unrecognised `schema_version` take their no-usable-prior-state path. `travel-db.json` is not in the trusted row: its only mention in this plugin is a prose note about the travel plugin's own use, not a read. `/workspace/group/memory/` is read **and** write, since `append-to-daily-log.py` writes `memory/daily/<UTC-today>.md`.
...
The owner-side migration is `scripts/migrate-addresses-block.py` — restamp the block, touch no values, add no `home_metro` line, refuse a stamp above what this writer knows rather than downgrade it. It is a script and not a paragraph of instructions because a fixed parse of a fixed block is the definition of deterministic, and it runs from Step 1 (Bootstrap), so the block migrates when the owner READS it rather than whenever someone next happens to edit the profile. Idempotent, so running it every session costs nothing. It also refuses to stamp a block missing a required key — a record claiming the current shape while missing it sends readers down their block-is-readable path to find nothing, which is worse than the honest old stamp. Seventeen tests pin it, and the key table now marks required versus optional.

`SKILL.md` also stopped paraphrasing its own scripts. It described `register-session.py`'s two-write atomicity, `append-to-daily-log.py`'s lock mechanics and date resolution, and the dedup predicate down to whitespace normalization — every one of them a second copy of what the script already says, in a file that loads on every session. What survives is the contract: what to run, what comes back, and the two outcomes that actually change what the agent does next (an unwritten sentinel re-bootstraps; an all-duplicates append is a valid no-op). The `session-start-auto-context` incident note went to where incident notes go.
