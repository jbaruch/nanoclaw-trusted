...
Picks up after the runtime's first-touch 👀 (see `jbaruch/nanoclaw-core` `rules/telegram-protocol.md`):

1. Note the `<message id="...">` for reply threading.
2. Optionally upgrade the reaction once you've inspected the request — `react_to_message` supersedes the runtime emoji.
