...
The universal post-action verification rule lives in `core/ground-truth.md` ("Verifying after a state change" section). That rule covers file writes, task schedules, API calls, memory updates, and IPC messages — all of which apply on this trust tier as well.
