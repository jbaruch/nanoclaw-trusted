...
| [identity-dual-handle](rules/identity-dual-handle.md) | Deploy-tier reference incident for the abstract dual-handle invariant in `nanoclaw-core`'s `core-behavior.md`: a concrete failure where the agent split itself into two addressees because one message used both its display-name trigger AND its Telegram `@username`. |
